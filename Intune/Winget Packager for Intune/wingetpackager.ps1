...
$uploadButton.Add_Click({
   ...
   #$WinGetAppSplit = $WinGetAppId.Replace('.','*')
    ...
    $RequirementScript = "`$WingetSystemPath = Get-ChildItem -Path 'C:\Program Files\WindowsApps' -Filter 'Microsoft.DesktopAppInstaller_*x64*' | Sort-Object -Property 'LastWriteTime' -Descending | Select-Object -First 1 -ExpandProperty 'FullName'

    If(Test-Path -LiteralPath `$WingetSystemPath\winget.exe){
    Write-Output 'winget installer present'
    Exit 0
    }else{
    Write-Output 'winget installer not present'
    Exit 1
    }"
...
#Create the install, uninstall, and detection script
...
$RequirementScript | Out-File -FilePath "$destPath\wingetcheck.ps1" -Encoding utf8
...
#Create the detection rule that checks for winget
#$RequirementRuleScript = New-IntuneWin32AppDetectionRuleScript -ScriptFile "$destPath\wingetcheck.ps1" -EnforceSignatureCheck $false -RunAs32Bit $false -OutVariable "String" -StringComparisonOperator "equal" -StringValue "winget installer present" 
...
Connect-MSIntuneGraph -TenantID $TenantName -Interactive
...
$appNametextBox.Text = ($AppSearchInfo.Name.Split('[')[0]).Trim()
...
try{
Start-Process -FilePath `$wingetSystemPath\winget.exe -ArgumentList 'install --scope machine -h --accept-package-agreements --accept-source-agreements $WinGetAppId' -Wait
}catch{
Start-Process -FilePath `$wingetSystemPath\winget.exe -ArgumentList 'install -h --accept-package-agreements --accept-source-agreements $WinGetAppId' -Wait
}
"
...
`$wingetcmd = '`$wingetSystemPath\winget.exe'
& `$wingetcmd list --accept-source-agreements | Out-Null
& `$wingetcmd uninstall --silent --accept-package-agreements --accept-source-agreements $WinGetAppId"

$detectionscripttextBox.Text = "`$64BitReg = Get-ChildItem 'HKLM:SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\' -Recurse | Get-ItemProperty | Where-Object {`$_.DisplayName -like '*$AppDisplayName*'}
